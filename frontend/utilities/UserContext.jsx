...
const UserProvider = ({ children }) => {
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const fetchUser = async () => {
      try {
          ...
          if (error.response && error.response.status === 401) {
              // Quietly handle 401 error and assume the user is not logged in
              console.log('User is not logged in.'); // Optionally log for debugging
              setUser(null);
          } else {
              // Log other errors, which are likely genuine issues
    ...
    //Function to sign up:
    const signup = async ( username, email, password, dob, timeOfBirth, locationOfBirth ) => {
      try {
        const response = await axios.post(`${apiUrl}/users`, {username, email, password, dob, timeOfBirth, locationOfBirth}, { withCredentials: true });
        setUser(response.data.user);
